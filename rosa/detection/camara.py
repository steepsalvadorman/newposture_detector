...
class HiloCamara(threading.Thread):
    ...
    def _evaluar_periodo(self, lms, w, h):
        buf = self._buf
        if not all(len(v) > 0 for v in buf.values()):
            return True
        ...
        try:
            inferidas, motivos_inferidos = _inferir_flags_ergonomicos(
                lms, w, h, self._ultimas_detecciones_obj, ang_medianas,
            )
            inferencia_mouse = _inferir_mouse_desde_pose_objetos(
                lms, w, h, self._ultimas_detecciones_obj,
            )

            pantalla_elevada = self._cfg_bool("pantalla_elevada") or inferidas["pantalla_elevada"]
            pantalla_baja = self._cfg_bool("pantalla_baja") or inferidas["pantalla_baja"]
            pantalla_dist_ok = (
                (self._cfg_bool("pantalla_dist_ok", True) or inferidas["pantalla_dist_ok"])
                and not pantalla_elevada
                and not pantalla_baja
            )
            hombros_encogidos_silla = self._cfg_bool("hombros_encogidos_silla") or inferidas["hombros_encogidos_silla"]
            reposabrazos_altos_bajos = self._cfg_bool("reposabrazos_altos_bajos") or inferidas["reposabrazos_altos_bajos"]
            alcance_sobre_cabeza = self._cfg_bool("alcance_sobre_cabeza") or inferidas["alcance_sobre_cabeza"]
            teclado_elevado_hombros = self._cfg_bool("teclado_elevado_hombros") or inferidas["teclado_elevado_hombros"]
            raton_alineado = self._cfg_bool("raton_alineado", True)
            ...
            resultado = calcular_ROSA_completo_v5(
                ang_tronco=ang_medianas[0],
                ang_rodilla=ang_medianas[1],
                ang_codo=ang_medianas[2],
                desv_cuello=ang_medianas[3],
                ang_muneca=ang_medianas[4],
                horas_silla=self._cfg_num("horas_silla"),
                horas_telefono=self._cfg_num("horas_telefono"),
                horas_pantalla=self._cfg_num("horas_pantalla"),
                horas_raton=self._cfg_num("horas_raton"),
                horas_teclado=self._cfg_num("horas_teclado"),
                pie_llega_suelo=self._cfg_bool("pie_llega_suelo", True),
                altura_regulable=self._cfg_bool("altura_regulable", True),
                espacio_insuficiente_piernas=self._cfg_bool("espacio_insuficiente_piernas"),
                dist_rodilla_cm=self._cfg_num("dist_rodilla_cm", 8.0),
                profundidad_regulable=self._cfg_bool("profundidad_regulable", True),
                tiene_reposabrazos=self._cfg_bool("tiene_reposabrazos", True),
                reposabrazos_ajustable=self._cfg_bool("reposabrazos_ajustable", True),
                bordes_afilados=self._cfg_bool("bordes_afilados"),
                brazos_anchos=self._cfg_bool("brazos_anchos"),
                reposabrazos_no_regulables=self._cfg_bool("reposabrazos_no_regulables"),
                reposabrazos_altos_bajos=reposabrazos_altos_bajos,
                usa_respaldo=self._cfg_bool("usa_respaldo", True),
                apoyo_lumbar_adecuado=self._cfg_bool("apoyo_lumbar_adecuado", True),
                hombros_encogidos_silla=hombros_encogidos_silla,
                respaldo_no_regulable=self._cfg_bool("respaldo_no_regulable"),
                telefono_alejado=self._cfg_bool("telefono_alejado"),
                sujecion_hombro_cuello=self._cfg_bool("sujecion_hombro_cuello"),
                sin_manos_libres=self._cfg_bool("sin_manos_libres"),
                pantalla_dist_ok=pantalla_dist_ok,
                pantalla_baja=pantalla_baja,
                pantalla_elevada=pantalla_elevada,
                dist_pantalla_mayor_75=self._cfg_bool("dist_pantalla_mayor_75"),
                giro_otra_pantalla=self._cfg_bool("giro_otra_pantalla"),
                sin_portadocumentos=self._cfg_bool("sin_portadocumentos"),
                pantalla_reflejos=self._cfg_bool("pantalla_reflejos"),
                raton_alineado=raton_alineado,
                agarre_pinza=self._cfg_bool("agarre_pinza"),
                raton_teclado_dif_altura=self._cfg_bool("raton_teclado_dif_altura"),
                reposamanos_duro=self._cfg_bool("reposamanos_duro"),
                desviacion_escribir=self._cfg_bool("desviacion_escribir"),
                alcance_sobre_cabeza=alcance_sobre_cabeza,
                teclado_elevado_hombros=teclado_elevado_hombros,
                sin_soporte_teclado=self._cfg_bool("sin_soporte_teclado"),
            )
        except Exception as e:
        ...
        resultado.update({
            "obj_pantalla": obj_pantalla,
            "obj_teclado": obj_teclado,
            "obj_mouse": obj_mouse,
            "mano_sobre_mouse": inferencia_mouse["mano_sobre_mouse"],
            "raton_alineado_eval": raton_alineado,
            "lado_mouse": inferencia_mouse["lado_mouse"],
            ...
            "flags_inferidos": inferidas,
            "motivos_inferidos": ", ".join(dict.fromkeys(motivos_inferidos)),
        ...
        try:
            ...
            while self.activo and not self._stop_event.is_set():
                ...
                if time.time() - last_eval >= INTERVALO_EVAL_SEG:
                    if not self._evaluar_periodo(lms, w, h):
                        break
                    last_eval = time.time()
        finally:
            lector = self._lector_frames
