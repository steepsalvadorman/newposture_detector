...
from rosa.export.excel import descargar_modelo
from rosa.core.calculos import C, MAX_CAMARAS_SCAN
...
class DialogoInicio(tk.Tk):
    ...
    def _verificar_modelo(self):
        def status(msg):
            self.after(0, lambda: self.lbl_modelo.config(text=msg))

        ok = descargar_modelo(status)
        if ok:
            self.after(0, self._on_modelo_listo)
        else:
            self.after(0, self._on_modelo_error)

    def _on_modelo_listo(self):
        self.lbl_modelo.config(text="Modelo listo", fg=C["ok"])
        self.btn_iniciar.config(state="normal")

    def _on_modelo_error(self):
        self.lbl_modelo.config(text="No se pudo descargar el modelo", fg=C["danger"])
    ...
    def _ok(self):
        ...
        if not desktop.exists():
            desktop = Path.home()
        ruta_xlsx = str(desktop / f"ROSA_{nombre.replace(' ', '_')}_{time.strftime('%Y%m%d_%H%M%S')}.xlsx")
